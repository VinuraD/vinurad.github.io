...
Now, let's do another intervention (do(Y=3)). This time however, we can observe and measure a difference. In SCM 2, $X$ will still follow its original distribution, while SCM_1 will show a mean shift from the previous intervened distribution. 

$$
P_{SCM_1}(X|do(Y=3)) \neq P_{SCM_1}(X|do(Y=2))\\
P_{SCM_2}(X|do(Y=3)) = P_{SCM_2}(X|do(Y=2))\\
$$
